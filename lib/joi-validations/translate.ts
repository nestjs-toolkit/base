...
const data = {
  pt_br: {
    ...
    string: {
      ...
      hostname: 'campo "{label}" não é um código válido',
    },
    number: {
      min: 'campo "{label}" deve conter um valor maior ou igual a {limit}',
      max: 'campo "{label}" deve conter um valor menor ou igual a {limit}',
      hostname: 'campo "{label}" não é um código válido',
    },
    boolean: { base: 'campo "{label}" não é do tipo boleano' },
    objectId: { invalid: 'campo "{label}" não é um UUID válido' },
  },
};
